...
> > ![[错题知识网络/assets/visual_wrong_questions/LA-116/solution_01.png]]
>
> 解析文字：先由 $\alpha\ne0$ 且不是 $A$ 的特征向量，推出 $\alpha,A\alpha$ 线性无关，所以 $P$ 可逆。再写 $AP=(A\alpha,A^2\alpha)$，由 $A^2\alpha=6\alpha-A\alpha$ 得到 $AP=PB$，其中 $B=\begin{pmatrix}0&6\\1&-1\end{pmatrix}$，于是 $P^{-1}AP=B$。最后用 $B$ 的两个不同特征值判断 $A$ 可相似对角化。
