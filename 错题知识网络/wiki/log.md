...
## 2026-09-11 ingest | 概率论与线性代数知识点锚点建设

参考包 `A-math-anchor-distribution-20260909-01`（五份讲义804页）本地正式落地。13条锚点复用本地已有规范键，2条按同义改写加别名（范德蒙德行列式→Vandermonde行列式；矩阵多项式的谱映射→矩阵多项式谱映射），259条新建为来源支撑的通用知识锚点页（MATHWIKI-KNOWLEDGE-530—788）。按原卡已记录的 knowledge／methods 判断普通涉及关系249条、覆盖97张线代正式卡；概率正式卡无匹配，未强行连线。本批不新建错题、不产生个人观察、不写评分或复习事件；184条关系仍为 accepted=false 提案。
