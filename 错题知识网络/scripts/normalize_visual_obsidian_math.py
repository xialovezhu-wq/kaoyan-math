...
def _repair_map() -> dict[str, tuple[Repair, ...]]:
    """Return exact, source-backed exceptions needed before lexical conversion."""

    return {
        "错题知识网络/可视化错题详情/高等数学/GS-447_58000-2026.5.13.md": (
            Repair(
                "restore-gs447-truncated-wrong-point",
                r"- 易错触发：把 B 选项 \(|x|\sin\sqrt{|x|}\) 的结构看错、拆错，误以为 \(\frac{|x|}{x}\) 出现就不可导；实际上 \(\frac{|x|}{x}\) 有界，且 \(\sin\sqrt{|x|}\to0\)，所以 B 的差商趋于 0。过早判定 B 后没有继续检查 D；D 中 \(\frac{\cos\sqrt{|x|}-1}{x}\...",
                r"- 易错触发：把 B 选项 \(|x|\sin\sqrt{|x|}\) 的结构看错、拆错，误以为 \(\frac{|x|}{x}\) 出现就不可导；实际上 \(\frac{|x|}{x}\) 有界，且 \(\sin\sqrt{|x|}\to0\)，所以 B 的差商趋于 0。过早判定 B 后没有继续检查 D；D 中 \(\frac{\cos\sqrt{|x|}-1}{x}\sim-\frac{|x|}{2x}\)，左右极限分别为 \(-\frac12\) 与 \(\frac12\)，才是真正不可导。",
                "错题知识网络/错题卡/GS-447_58000零点可导性判定.md:25",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-485_102454-2026.5.21.md": (
            Repair(
                "restore-gs485-truncated-wrong-point",
                r"- 易错触发：比较方向判断反了；由 \(u_{n+1}\le u_n\) 应推出 \(u_{n+1}\le\sqrt{u_nu_{n+1}}\le u_n\)，真正可用的是左半边 \(u_{n+1}\le\sqrt{u_nu_{n+1}}\)。同时把“通项相乘”与“目标级数收敛”混淆，即使能讨论 \(\sum u_nu_{n+1}\)，也不能直接推出 \(\sum u_n...",
                r"- 易错触发：比较方向判断反了；由 \(u_{n+1}\le u_n\) 应推出 \(u_{n+1}\le\sqrt{u_nu_{n+1}}\le u_n\)，真正可用的是左半边 \(u_{n+1}\le\sqrt{u_nu_{n+1}}\)。同时把“通项相乘”与“目标级数收敛”混淆，即使能讨论 \(\sum u_nu_{n+1}\)，也不能直接推出 \(\sum u_n\) 收敛。",
                "错题知识网络/错题卡/GS-485_102454单调错位比较.md:31",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-501_19516-2026.5.26.md": (
            Repair(
                "restore-gs501-truncated-wrong-point",
                r"- 易错触发：看到 \((-1)^n\) 后直接想套莱布尼茨判别法，未先检查正项部分 \(a_n=\frac1{\sqrt n+(-1)^n}\) 是否单调递减；本题正项部分因分母中的 \((-1)^n\) 奇偶跳变而不单调，必须先有理化拆成收敛交错部分与发散调和部分。另一个细节是：\(\sqrt{n+1}+\sqrt n>1\)，所以 \(\frac1{\sqrt{n+...",
                r"- 易错触发：看到 \((-1)^n\) 后直接想套莱布尼茨判别法，未先检查正项部分 \(a_n=\frac1{\sqrt n+(-1)^n}\) 是否单调递减；本题正项部分因分母中的 \((-1)^n\) 奇偶跳变而不单调，必须先有理化拆成收敛交错部分与发散调和部分。另一个细节是：\(\sqrt{n+1}+\sqrt n>1\)，所以 \(\frac1{\sqrt{n+1}+\sqrt n}<1\)，不等号方向不能反。",
                "错题知识网络/错题卡/GS-501_19516交错级数有理化拆项.md:36",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-516_79209-2026.5.30.md": (
            Repair(
                "restore-gs516-truncated-wrong-point",
                r"- 易错触发：用比值审敛法求含参正项级数收敛域时，算到 $\frac{u_{n+1}}{u_n}=e^{-x}\left(\frac{n}{n+1}\right)^n$，因 $\frac{n}{n+1}\to1$ 就把 $\left(\frac{n}{n+1}\right)^n$ 直接判成 $\to1$。这是典型 $1^\infty$ 型，不能直接代极限，必须改写成 $\...",
                r"- 易错触发：用比值审敛法求含参正项级数收敛域时，算到 $\frac{u_{n+1}}{u_n}=e^{-x}\left(\frac{n}{n+1}\right)^n$，因 $\frac{n}{n+1}\to1$ 就把 $\left(\frac{n}{n+1}\right)^n$ 直接判成 $\to1$。这是典型 $1^\infty$ 型，不能直接代极限，必须改写成 $\dfrac1{(1+\frac1n)^n}\to\dfrac1e$，正确得 $\rho=e^{-1-x}$；少这一步，收敛域会整体平移一个单位。$\rho=1$（即 $x=-1$）时比值法失效，须代回原级数，由通项 $v_n=\frac{n!}{n^n}e^n$ 单调递增不趋 0、用级数收敛必要条件判发散。",
                "错题知识网络/错题卡/GS-516_79209含参正项级数收敛域.md:29",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-518_102395-2026.5.30.md": (
            Repair(
                "restore-gs518-truncated-wrong-point",
                r"""- 易错触发：系数为"两个不同性质表达式之和"的幂级数，端点处整体比值法失效后没有拆开判断。正确做法：拆成 $\sum_{n\ge2}\frac{x^n}{n\ln n}$（$R=1$，$x=1$ 处为广义调和 $\sum\frac1{n\ln n}$（$p=1$）发散，$x=-1$ 处交错由莱布尼茨收敛，收敛域 $[-1,1)$）与 $\sum_{n\ge2}\frac...""",
                r"""- 易错触发：系数为"两个不同性质表达式之和"的幂级数，端点处整体比值法失效后没有拆开判断。正确做法：拆成 $\sum_{n\ge2}\frac{x^n}{n\ln n}$（$R=1$，$x=1$ 处为广义调和 $\sum\frac1{n\ln n}$（$p=1$）发散，$x=-1$ 处交错由莱布尼茨收敛，收敛域 $[-1,1)$）与 $\sum_{n\ge2}\frac{x^n}{2^n}$（等比型，收敛域 $(-2,2)$），再取交集 $[-1,1)\cap(-2,2)=[-1,1)$。关键：端点整体判别法失效时，回到原级数把通项拆成已知类型级数（广义调和 / 等比 / 交错）分别判。""",
                "错题知识网络/错题卡/GS-518_102395和型幂级数拆分收敛域.md:30",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-484_102447-2026.5.21-2026.5.26.md": (
            Repair(
                "wrap-gs484-bare-latex",
                r"- 证据边界：个人断点只认“没有从正项级数收敛提取 (a_n\to0) 与最终 (0<a_n<1)，因而未建立 (a_n^2\le a_n) 的比较”；未发现同对象、同第一动作且双端均有用户证据的卡片。",
                r"- 证据边界：个人断点只认“没有从正项级数收敛提取 $a_n\to0$ 与最终 $0<a_n<1$，因而未建立 $a_n^2\le a_n$ 的比较”；未发现同对象、同第一动作且双端均有用户证据的卡片。",
                "错题知识网络/错题卡/GS-484_102447正项级数平方比较.md:36",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-662_57909复合三角积分比较.md": (
            Repair(
                "undouble-gs662-inline-math",
                r"引入 \\(x\\) 作中间量",
                r"引入 \(x\) 作中间量",
                "正文 line 75 的双重转义公式",
            ),
            Repair(
                "undouble-gs662-inline-math-2",
                r"尝试换元，把问题带到 \\(\\arcsin t\\) 的复杂路径",
                r"尝试换元，把问题带到 \(\arcsin t\) 的复杂路径",
                "正文 line 75 的双重转义公式",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-663_81436定积分比较作差反折.md": (
            Repair(
                "undouble-gs663-inline-math",
                r"没有把 \\(I_1-I_2\\) 落地；比较分母时忘记倒数不等号反向；反折换元时把 \\(\\frac\\pi2-\\frac\\pi4\\) 算成负数",
                r"没有把 \(I_1-I_2\) 落地；比较分母时忘记倒数不等号反向；反折换元时把 \(\frac\pi2-\frac\pi4\) 算成负数",
                "正文 line 97 的双重转义公式",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-664_170728凹函数弦线积分均值.md": (
            Repair(
                "undouble-gs664-inline-math",
                r"没有从 \\(F'\\) 与 \\(F''\\) 的符号触发凹函数弦线图像比较；转而构造 \\(g(x)\\) 后，又把全区间积分常数在 \\(x=0\\) 处误消成 0",
                r"没有从 \(F'\) 与 \(F''\) 的符号触发凹函数弦线图像比较；转而构造 \(g(x)\) 后，又把全区间积分常数在 \(x=0\) 处误消成 0",
                "正文 line 91 的双重转义公式",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-170_强化例题8.1.md": (
            Repair(
                "remove-gs170-flattened-blockquote-markers-1",
                r"\[ > \boxed{ > \lim_{n\to\infty}\sum_{k=1}^{n}f\!\left(\frac{k}{n}\right)\frac{1}{n} > =\int_{0}^{1}f(x)\,dx > } > \]",
                r"\[ \boxed{\lim_{n\to\infty}\sum_{k=1}^{n}f\!\left(\frac{k}{n}\right)\frac{1}{n}=\int_{0}^{1}f(x)\,dx} \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
            Repair(
                "remove-gs170-flattened-blockquote-markers-2",
                r"\[ > \lim_{n\to\infty}\sum_{i=1}^{n} f\!\left(a+\frac{b-a}{n}\,i\right)\frac{b-a}{n} > = \int_{a}^{b} f(x)\,dx. > \]",
                r"\[ \lim_{n\to\infty}\sum_{i=1}^{n} f\!\left(a+\frac{b-a}{n}\,i\right)\frac{b-a}{n} = \int_{a}^{b} f(x)\,dx. \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-442_57797-2026.5.11.md": (
            Repair(
                "remove-gs442-flattened-blockquote-markers-1",
                r"\[ > \boxed{ > \lim_{n\to\infty}\sum_{k=1}^{n}f\!\left(\frac{k}{n}\right)\frac{1}{n} > =\int_{0}^{1}f(x)\,dx > } > \]",
                r"\[ \boxed{\lim_{n\to\infty}\sum_{k=1}^{n}f\!\left(\frac{k}{n}\right)\frac{1}{n}=\int_{0}^{1}f(x)\,dx} \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
            Repair(
                "remove-gs442-flattened-blockquote-markers-2",
                r"\[ > \lim_{n\to\infty}\sum_{i=1}^{n} f\!\left(a+\frac{b-a}{n}\,i\right)\frac{b-a}{n} > = \int_{a}^{b} f(x)\,dx. > \]",
                r"\[ \lim_{n\to\infty}\sum_{i=1}^{n} f\!\left(a+\frac{b-a}{n}\,i\right)\frac{b-a}{n} = \int_{a}^{b} f(x)\,dx. \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
        ...
        "错题知识网络/可视化错题详情/高等数学/GS-419_强化例题12.4.md": (
            Repair(
                "remove-gs419-flattened-blockquote-markers",
                r"\[ > g = 9.8\,\text{m/s}^2 > \]",
                r"\[ g = 9.8\,\text{m/s}^2 \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
        ...
        "错题知识网络/可视化错题详情/高等数学/GS-420_2020年第12题.md": (
            Repair(
                "remove-gs420-flattened-blockquote-markers",
                r"\[ > g = 9.8\,\text{m/s}^2 > \]",
                r"\[ g = 9.8\,\text{m/s}^2 \]",
                "压平来源中混入公式载荷的 blockquote 标记",
            ),
        ...
        "错题知识网络/可视化错题详情/线性代数/MN4-GS-CH01-651_强化例题1.4.md": (
            Repair(
                "structure-la651-modern-display-callout-fragment",
                r""">  > ✅ 快验算： > 当 $n=2$ 时， >
>
> $$
...
>
> ### 3. 方法小结""",
                "现代公式及相邻段落中误混入的压平 blockquote 标记",
            ),
        ),
        "错题知识网络/可视化错题详情/高等数学/GS-126_25882-2026.2.28-T1.md": (
            Repair(
                "structure-gs126-modern-display-callout-fragment",
                r"""> > 提示：同样先得 $e^y=2x+\sqrt{4x^2+1}$，再写 >
>
> $$
> > e^{-y}=\sqrt{4x^2+1}-2x >
> $$
...
> 相减立刻得到 $4x=e^y-e^{-y}$。""",
        ...
        "错题知识网络/可视化错题详情/高等数学/GS-413_强化例题14.8.md": (
            Repair(
                "structure-gs413-modern-display-callout-fragment",
                r""">  > 两次换元的总雅可比为 >
>
> $$
...
KNOWN_OUT_OF_SCOPE_TRUNCATIONS = (
