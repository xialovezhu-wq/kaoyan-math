...
def _build(root: Path, conn: sqlite3.Connection, affected_ids: list[str] | None) -> dict:
    ...
    with conn:
        for table in ("meta", "sources", "concepts", "lookup", "records", "membership"):
...
def resolve_concepts(repo_root: str | Path, concepts: list[str], view: str = "protected",
    ...
    try:
        ...
        for key in sorted(keys):
            ...
            resolved.append({k: v for k, v in concept.items() if k not in {"formal_ids", "source_refs"}} | {"formal_count": len(concept["formal_ids"])})
