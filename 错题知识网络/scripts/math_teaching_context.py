...
def resolve_teaching_concepts(repo_root: Path | str, concepts: list[str],
    ...
    return math_concept_index.resolve_concepts(
        repo_root, concepts, view, max_bytes, offset,
        material_limit=material_limit, record_projection=teaching_record_projection)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    resolve.add_argument("--max-bytes", type=int, default=10000,
                         help="Combined UTF-8 budget for concept history (default 10000)")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "materialize":
            ...
            if args.concepts:
                result = resolve_teaching_concepts(
