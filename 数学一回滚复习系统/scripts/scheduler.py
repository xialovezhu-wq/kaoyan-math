        ...
        if mode in {
            ...
            lines.extend(
                [
                    ...
                    f"- 评分命令：`python3 数学一回滚复习系统/scripts/scheduler.py score-warmup {queue_id} {candidate.get('queue_item_id')} 分数 --date {current_date.isoformat()}`",
                    "",
                ]
            )
        else:
            lines.extend(
                [
                    "本题尚未正式到期，仅作补位训练；不得调用 score，不推进长期阶段。",
                    "",
                ]
            )
